 ...
 * 小型婚礼
 */
var congratulations = [{'con':'一定要幸福哦1','name':'陈宁1'},
                       {'con':'一定要幸福哦2','name':'陈宁2'},
                       {'con':'一定要幸福哦3','name':'陈宁3'},
                       {'con':'一定要幸福哦4','name':'陈宁4'},
                       {'con':'一定要幸福哦5','name':'陈宁5'},
                       {'con':'一定要幸福哦6','name':'陈宁6'}
];
$(function(){
    ...
    function scene2(){
        ...
        $firstHorn.animate({left: "-422px"},function(){
            ...
            $secondTxt.animate({left: "430px"},function(){
    ...
    function scene7(){
        ...
        $.each(congratulations,function(n,value) {
            var _div = "";
            _div += '<div class="note-'+Math.ceil(Math.random()*6);+'">'+value.con+'<br>'+value.name+'</div>';
            _divs += _div;
        });
        $sevenContent.append(_divs); /* 如果输入祝福语，将此标签添加的尾部 */
        ...
        $uSure.click(function(){
            ...
            var _invname = $inv_name.val();
            var _randomNum = Math.ceil(Math.random()*6);
            if(_writeVal != "送上您的祝福吧~"){
                var _div = '<div class="note-'+_randomNum+'">'+_writeVal+'<br>'+_invname+'</div>';
