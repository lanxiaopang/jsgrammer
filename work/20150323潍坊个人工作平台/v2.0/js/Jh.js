/*
 ...
 @author xiaofan
 @modifier SunZhongyi szy1944@hotmail.com
 */
var Jh = {
    Config:{//CLASS样式配置
        tableCls:"form-list",
...
Jh.Util = {//工具类
    ...
    toSomething:function(o,num){
        if(num==1){
            $(".main_wrap").append(o);
        }else if(num==2){
            $(".skin_wrap").append(o);
        }else if(num==3){
            $(".layout_wrap").append(o);
        }
    }
...
Jh.fn = function(me){//功能区
    return me = {
        ...
        _create:function(o){//创建自己
            var _box = $("<div id='header1'/>");
            me.box = _box;
            //Jh.Util.toSomething(_box);//往Body里添加自己
            Jh.Util.toSomething(_box,1);//往Body里添加自己
            //Jh.Util.toSomething("aaaa",2);//往Body里添加自己
        },

        _createWrap:function(d){//创建外层容器
            var _table = me._createTable(Jh.Config.tableCls);
            me._ele.table = _table;
            me._createModuleList(d);//功能模块设置
            me._createActionButton();//按钮
            me._addPanel(_table);
        },
        ...
        _createTable:function(clsName){	//创建表格
            ...
            $("<tbody/>").append(me._createLayoutTr(2))
                .append(me._createBaseTr())
                .append(me._createActionTr())
                .appendTo(_t);
            return _t;
        },
        _createTable2:function(clsName){	//创建表格
            var _t = $("<table/>").addClass(clsName);
            $("<tbody/>").append(me._createLayoutTr(1))
                //.append(me._createBaseTr())
                //.append(me._createActionTr())
                .appendTo(_t);
            return _t;
        ...
        _createBaseTr:function(){//创建功能模块tr
            var	_td = me._createTd(Jh.Config.tdCls2),
                _tr = $("<tr>").append(me._createTd(Jh.Config.tdCls,"模块定制:"))
                    .append(_td);
            me._ele.mtd = _td;
            return _tr;
        },
        ...
        _createLayoutTr:function(n){//创建布局tr
            ...
            if(n==1){
                    ...
                    //.append("<a href='javascript:void(0);' class='active' rel='1:1:1'>\u9ED8\u8BA4</a>")
                       ...
                       _div.append("<a href='javascript:void(0);' rel='50:77'>两栏布局</a>")
                    /*.append(me._createA("两栏布局"))*/
                    .append("<a href='javascript:void(0);' class='active' rel='30:57:39'>三栏布局</a>")
        ...
        _ToLayout:function(v){//刷新布局
            ...
            $.each(ModulesId, function(s, sn){
                ...
                currentModule.removeClass("w250 w750 w500 wnone w300 w390 w770 w570").addClass(ary[s]);//增加css
            });
...
Jh.Portal = function(me){//Portal对象
    var _box = "<div id='portal'></div>",
        _template = {//模板
            l :"<div id='"+Jh.Layout.location.left+"' class='"+Jh.Config._groupWrapperClass+" w300'/>",
            m :"<div id='"+Jh.Layout.location.center+"' class='"+Jh.Config._groupWrapperClass+" w570'/>",
            r :"<div id='"+Jh.Layout.location.right+"' class='"+Jh.Config._groupWrapperClass+" w390'/>",
    ...
    return me={
        ...
        _createItemContent:function(){//创建content
            ...
            $("<ul style='width:100%; height:250px;'><li></li></ul>").appendTo(_content);
            return _content;
        },
