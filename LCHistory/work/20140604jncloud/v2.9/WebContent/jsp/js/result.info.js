function getShowInfoNew(dataUrls,pk,docs,i){
	...
	if(dataUrls=="peopleidx"){
		var showCym = n2b(docs[i].cym,"");
		var imageUrl = contPath+'/img/peoplePhoto/'+n2bAddRep(docs[i].id,"")+'?s=2';
  		//imageUrl = contPath+'/images/result/touxiang.jpg';
		
		
		innerStr +='<div class="result_list_li border_bot_dot">';
		innerStr +='<div class="li_title">';
		innerStr +='<span class="tit_left">';
		innerStr +='<a href="/a/people/'+n2bAddRep(docs[i].id,"")+'"  target="_blank">'+ n2bAddRep(docs[i].xm,"")+'('+n2bAddRep(docs[i].id,"")+')的人员档案信息'+'</a>';
		innerStr +='</span>'; 
		innerStr +='</div>';
	
		innerStr +='<div class="li_body">';
			innerStr +='<div class="li_img"><img src="'+imageUrl+'"></div>';
				innerStr +='<div class="li_detail">';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('姓名',n2bAddRep(docs[i].xm,""))+'</span>';
					if(showCym.length>0){
						innerStr +='<span class="li_detail_item">'+getAddHtmInfo('曾用名',n2bAddRep(showCym,""))+'</span>'; 
				  	}
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('性别',n2bAddRep(docs[i].xb,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('身份证号',n2bAddRep(docs[i].id,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('民族',n2bAddRep(docs[i].mz,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('职业',n2bAddRep(docs[i].zy,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('联系电话',n2bAddRep(docs[i].dh,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('户籍地址',n2bAddRep(docs[i].dz,""))+'</span>';
					innerStr +='<span class="li_detail_item">'+getAddHtmInfo('工作单位',n2bAddRep(docs[i].gzdw,""))+'</span>';
					//innerStr +='<span class="li_detail_item">'+'<br>'+'</span>';
					//innerStr +='<span class="li_detail_item green_color1">'+'所属派出所：'+ n2b(docs[i].pcs,"")+'</span>';
					//innerStr +='<span class="li_detail_item green_color1">'+'所属辖区：'+ n2b(docs[i].xq,"")+'</span>'; 
				innerStr +='</div>';
			innerStr +='</div>';	
		innerStr +='</div>';			
	innerStr +='</div>'; 
	...
	if(dataUrls=="hzczrk"){
		...
		if(zpid==""||zpid==undefined||zpid=="undefined"||zpid==null||zpid=="null"){
			imageUrl = contPath+'/images/result/touxiang.jpg';
      		...
      		imageUrl = contPath+'/images/result/touxiang.jpg';
      		//imageUrl = contPath+'/img?KEY='+zpid+'&DATATYPE=HZCZRK_IMG&s=0';
      	}
		
		innerStr +='<div class="result_list_li border_bot_dot">';
			innerStr +='<div class="li_title">';
			innerStr +='<span class="tit_left">';
			innerStr +='<a href="javascript:;" onclick=openDetail(\"'+dataUrls+'\",\"'+pk+'\")>'+ n2b(docs[i].XM,"")+'('+n2b(docs[i].GMSFHM,"")+')的常住人口信息'+'</a>';
			innerStr +='</span>';
			innerStr +=addLookPeopleIdx(docs[i].GMSFHM);
			innerStr +='</div>';
		
			innerStr +='<div class="li_body">';
				innerStr +='<div class="li_img"><img src="'+imageUrl+'"></div>';
					innerStr +='<div class="li_detail">';
						innerStr +='<span class="li_detail_item">'+'姓名：'+ n2b(docs[i].XM,"")+'</span>';
						if(showCym.length>0){
							innerStr +='<span class="li_detail_item">'+'曾用名：'+ showCym+'</span>'; 
					  	}
						innerStr +='<span class="li_detail_item">'+'性别：'+ n2b(docs[i].XB,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'身份证号：'+ n2b(docs[i].GMSFHM,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'民族：'+ n2b(docs[i].MZ,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'职业：'+ n2b(docs[i].ZY,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'户籍地址：'+ n2b(docs[i].ZZ,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'服务处所：'+ n2b(docs[i].FWCS,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'人员状态：'+ n2b(docs[i].ZXBZ,"")+'</span>';
						innerStr +='<span class="li_detail_item">'+'<br>'+'</span>';
						innerStr +='<span class="li_detail_item green_color1">'+'所属派出所：'+ n2b(docs[i].PCS,"")+'</span>';
						innerStr +='<span class="li_detail_item green_color1">'+'所属辖区：'+ n2b(docs[i].BZXQ,"")+'</span>'; 
					innerStr +='</div>';
				innerStr +='</div>';	
			innerStr +='</div>';	
	...
	if(dataUrls=="hzzzrk"){
  		...
  		innerStr = innerStr + addLookPeopleIdx(docs[i].ZJHM);
...
function addLookPeopleIdx(zjhmObj){
	var zjhm = zjhmObj;
	var str = "";
  	if(zjhm!=undefined&&zjhm!=null&&zjhm!=""){
  		str +='<span class="tit_right">'; 
  		str +='&nbsp;&nbsp;<a href="'+contPath+'/a/people/'+changeNoLightFormat(zjhm)+'"  target="_blank"><span class="underline">查看人员档案</span></a>'; 
  		str +='</span>';  
  	}
  	return str;
}
